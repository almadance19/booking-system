      ...
      for (var i = 0; i < list.length; i++) { 
        if (list[i].id == val[0]) {
          ...
          if (val[7] === "NO" & val[2] == "0") {
            ...
            inputEl.value = "Free Registration";
            inputEl.style = "margin: 0px 4px 8px 3px;";
            
      
            inputEl.addEventListener('click', function() { 
              Create_Payment_Free(val[0], val[2], val[5], val[6], val[4], val[8]); 
            });
      
            container.appendChild(inputEl); 
          }
          else if (val[7] === "NO" & val[2] !== "0") {
            // Add One-time payment button only if val[7] is "NO"
            var inputEl = document.createElement('input'); 
            inputEl.type = 'button';
            inputEl.className = "btn btn-colour-1"; 
            ...
            monthlyEl.value = "Monthly Payment";
...
async function Create_Payment_Free(membership, price_total,nr_months,stripe_link,nr_courses,paypal_sub_id) { 
  await sleep(1000);
  
  paymentForm(membership, price_total,nr_months,stripe_link,"One-time payment",nr_courses,"Free",paypal_sub_id); 
  document.getElementById("stripe-container").style.display = 'none';
  document.getElementById("payment-block").style.display = 'block';
...
function paymentForm(membership, price_total,nr_months,stripe_link,payment_type,nr_courses,type_payment,paypal_sub_id)
{
  ...
  // If the payment type is "Free", we don't need to show the payment form
  else if (type_payment == "Free") {
    var script1 = document.createElement('script');
    script1.src = 'https://www.paypal.com/sdk/js?client-id=Ad_vdC1plSPko5d9oY4CJeobC3WYQqtoyOthXJ0IcKfo38y-Ya9wcgRTglhuWrNSRyub31mxXLhUuAFM&enable-funding=venmo&currency=EUR';
    // TEST script1.src = 'https://www.paypal.com/sdk/js?client-id=AYz2EWPGuZ55NZIQNKS8yb1NpbvPP2G11aIaMHAzRw2Wk2ACO7tPRI41JbRQ7nfNB0MchqrFS7m8aRzy&enable-funding=venmo&currency=EUR';
    ...
    displayTable += '</div>';
    displayTable += '<div class="form-group col-md-3">';
    displayTable += "<input type=\"button\" value=\"Registration\"  style=\"display:block\"  id=\"blockPrice\" class=\"btn btn-dark\" ";
    displayTable += " onclick=\"aggregatorProcess()\" />";
    displayTable += '</div>';
    displayTable += '<div id="display_error_pay" style="color: red" ></div>';
    displayTable += '<div class="form-group col-md-3">';
    displayTable += "<a class=\"btn btn-dark\" href=\""+stripe_link+"\" id=\"addStripe\" style=\"display:none;text-align:left\">Pay with Stripe</a>";
  ...
  // If the payment type is not "Total", we assume it's a free membership or a different payment method
    else {
...
function aggregatorProcess() {
  ...
  console.log("Aggregator process")
  document.querySelector(".section-1").style.display = 'block';
  var firstname_pay = document.getElementById("firstname_pay").value;
  var membershiptype =  document.getElementById("membership").value;
  var course_price = document.getElementById("price_course").value;
...
var date5 = new Date();
  ...
  var displayTable = ""
    ...
    displayTable += "<div class=\"col\" style=\"font-weight: bold\" >";
    displayTable += "<h3 id=\"paymentTitle\">Payment Details</h3>";
    displayTable +="<p style=\"font-size: 14px; line-height: 160%; text-align: left;\">Many thanks for registering "+firstname_pay+"</p>";
displayTable +="<p style=\"font-size: 14px; line-height: 160%; text-align: left;\"> </p>";
displayTable +="<p style=\"font-size: 14px; line-height: 160%; text-align: left;\">Course(s): <strong>"+course_pay+" </strong>.</p>";
displayTable +="<p style=\"font-size: 14px; line-height: 160%; text-align: left;\">Membership Type: <strong>"+membershiptype+"</strong>.</p>";
displayTable +="<p style=\"font-size: 14px; line-height: 160%; text-align: left;\"> </p>";
displayTable +="<p style=\"font-size: 14px; line-height: 160%; text-align: left;\">This is a free Registration valid for the following period: <strong>"+membershiptype_nr+" months</strong>.</p>";
displayTable +="<p style=\"font-size: 14px; line-height: 160%; text-align: left;\">Start Date: <strong>"+currentDate+"</strong>.</p>";
displayTable +="<p style=\"font-size: 14px; line-height: 160%; text-align: left;\">End Date: <strong>"+contract_date+"</strong>.</p>";
displayTable +="<p style=\"font-size: 14px; line-height: 160%; text-align: left;\">Name: <strong>"+firstname_pay+"</strong>.</p>";
displayTable +="<p style=\"font-size: 14px; line-height: 160%; text-align: left;\">Email: <strong>"+email_payment+"</strong>.</p>";
displayTable +="<p style=\"font-size: 14px; line-height: 160%;text-align: left;\"> </p>";
displayTable +="<p style=\"font-size: 14px; line-height: 160%; text-align: left;\">Best regards,</p>";
displayTable +="<p style=\"font-size: 14px; line-height: 160%; text-align: left;\">Eduardo &amp; Natalia<br />Alma Dance Team</p>";
// displayTable += "<input type=\"button\" value=\"Confirm Bank Payment Method\" style=\"background-color: rgb(172, 22, 22);\"  id=\"sendPaymentEmail2\" class=\"btn btn-dark\" ";
...
if(type_payment=="Monthly") {
  ...
  console.log("sending pre bank cash payment");
  document.getElementById("sendPaymentEmail").disabled = true;
  document.getElementById("paymentTitle").innerText = 'All registration details have been sent to your email. Please check your spam folder as well, dont repeat this step.';
  document.getElementById("paymentTitle").style = "color: red;font-weight:bold";
